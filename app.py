...
def load_resources():
    ...
    if os.path.exists(SITE_INDEX_FILE):
        try:
            ...
            logger.info(f"✅ Индекс сайта загружен: {len(site_index['pages'])} страниц, {len(site_index['documents'])} документов")
        except Exception as e:
...
def find_link_in_index(query: str) -> list:
    ...
    for doc in site_index.get('documents',[]):
        ...
        if query_lower in name:
            results.append({'title': doc.get('name', 'Документ'), 'url': doc.get('url', ''), 'type': doc.get('type', 'DOC').lower()})
    return results[:5]
